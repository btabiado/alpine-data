...
Plus: insights bar (rule-based, ~12 live notifications), a Claude-powered **Ask the data** chat dock (right side), **Symbol search**: type any ticker (BTC, NVDA, SOL, AAPL...) in the header search box → a universal modal shows the signal score, POC if available, recent news, and sentiment for that symbol. Any crypto symbol not already in the cached top-25 is fetched **LIVE from CryptoCompare** client-side so the lookup covers the long tail (SHIB, INJ, FET, ...). Stock tickers beyond the top-20 most-active require running the dashboard locally with `python server.py` — the page calls the new `/api/symbol/<symbol>` endpoint, which routes the lookup through Yahoo Finance server-side (Yahoo has no usable browser CORS). Optional HTTP Basic Auth, GitHub Pages mirror, Tailscale-ready.
