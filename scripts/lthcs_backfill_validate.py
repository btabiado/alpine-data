#!/usr/bin/env python3
"""LTHCS backfill validation runner.

Audits the completeness and consistency of a backfill run. READ-ONLY:
this script never writes to ``data/lthcs/`` source data, never touches
...
The script also writes a machine-readable JSON report to
``<data-root>/backfill_validation_<UTC timestamp>.json`` for later
analysis.
...
class Finding:
    ...
    are kept so the rendered report still says "✓ 137 / 137 ...".
    """
...
def _band_for_score(score: float, score_bands: dict) -> str | None:
...
def is_backfilled_snapshot(snapshot: dict, variable_detail: dict | None = None) -> bool:
            ...
            if expected and stats["variable_detail_rows"] != expected:
                report.add(Finding(
                    ...
                    message=(
                        ...
                        f"expected {expected} (={len(scores_rows)} tickers × {len(PILLAR_NAMES)} pillars)"
                    ),
                    detail={
                        "date": date_str,
...
def run_validation(
    ...
    rng_seed: int = 0,
) -> tuple[Report, list[dict], dict, list[str]]:
