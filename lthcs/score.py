...
def compute_volatility_modifier(
    ...
    """Return -3.0 if ticker is above the universe's 90th percentile.

    ``None`` ticker volatility or empty universe both return 0.0.
    Strict comparison (``> p90``) -- a ticker exactly at the threshold
    is not penalised.
    """
    ...
    if not universe_volatilities:
        return 0.0
    p90 = _percentile(list(universe_volatilities), _VOLATILITY_PERCENTILE)
    if p90 is None:
        return 0.0
    if ticker_v > p90:
        return _VOLATILITY_MAGNITUDE
    return 0.0
...
def compute_lthcs_score(
    ...
    *,
    ...
    vol_mod = compute_volatility_modifier(
        ticker_volatility, list(universe_volatilities or [])
    )
