...
.hbtn{white-space:nowrap;color:var(--text)}
@media (max-width:820px){.hbtn{min-height:44px;min-width:44px;padding:10px 14px}}
@media (max-width:420px){.hbtn{padding:10px 12px}.hbtn .hbl{display:none}}
...
<header class="top">
  ...
  <button class="chip hbtn" id="shareBtn" type="button" aria-label="Share this page">↗<span class="hbl"> Share</span></button>
  <button class="chip hbtn" id="textBtn" type="button" hidden aria-label="Text a link to this page">💬<span class="hbl"> Text</span></button>
</header>
...
const LS_URL_KEYS=['pres','rel','tier','seg','geo','type','q'];
// mirror the directory filters into the query string (replaceState — no history entries; empty params omitted)
let _suT=null;
function syncURL(){clearTimeout(_suT);try{const p=new URLSearchParams();LS_URL_KEYS.forEach(k=>{if(lsState[k])p.set(k,lsState[k]);});const qs=p.toString();history.replaceState(null,'',location.pathname+(qs?'?'+qs:'')+location.hash);}catch(e){}}
// debounced variant for per-keystroke callers — Safari rate-limits replaceState (~100/30s)
function syncURLSoon(){clearTimeout(_suT);_suT=setTimeout(syncURL,250);}
...
function lsKpis(){
  ...
  document.querySelectorAll('#lsKpis .kpi').forEach(b=>b.addEventListener('click',()=>{
    ...
    lsState.limit=120;LS_TABLE._render();syncURL();
  }));
}
...
function applyLS(scroll){lsState.limit=120;LS_TABLE._render();syncURL();if(scroll){const p=document.getElementById('lsTable').closest('.panel');if(p)p.scrollIntoView({behavior:'smooth',block:'start'});}}
...
function initLandscape(){
  ...
  document.getElementById('lsSeg').addEventListener('change',e=>{lsState.seg=e.target.value;lsState.limit=120;LS_TABLE._render();syncURL();});
  document.getElementById('lsGeo').addEventListener('change',e=>{lsState.geo=e.target.value;lsState.limit=120;LS_TABLE._render();syncURL();});
  document.getElementById('lsType').addEventListener('change',e=>{lsState.type=e.target.value;lsState.limit=120;LS_TABLE._render();syncURL();});
  document.querySelectorAll('#lsFilters .chip[data-rel]').forEach(c=>c.addEventListener('click',()=>{
    ...
    document.querySelectorAll('#lsKpis .kpi').forEach(x=>x.classList.remove('on'));lsState.limit=120;LS_TABLE._render();syncURL();}));
  ...
  document.getElementById('lsClear').addEventListener('click',()=>{
    ...
    document.querySelectorAll('#lsFilters .chip[data-tier]').forEach(x=>x.setAttribute('aria-pressed',String(x.dataset.tier==='')));
    LS_TABLE._render();syncURL();
  });
  // deep-link: /landscape/?pres=&rel=&tier=&seg=&geo=&type=&q= opens the directory pre-filtered
  // (unknown values ignored). Required by the header Share/Text buttons: shared URLs carry the
  // live filter state so the recipient lands on the same view; the Summit dashboard's
  // "?pres=absent" entry links also promise the filtered field in their labels.
  try{
    const sp=new URLSearchParams(location.search);
    const pp=sp.get('pres');if(pp==='present'||pp==='absent')lsState.pres=pp;
    const rr=sp.get('rel');if(rr&&RELK.includes(rr))lsState.rel=rr;
    const tt=sp.get('tier');if(tt&&['1','2','3'].includes(tt)&&lsState.pres==='absent')lsState.tier=tt; // tier only applies to the absent set (syncTierGroup would clear it anyway)
    [['seg','lsSeg'],['geo','lsGeo'],['type','lsType']].forEach(([k,id])=>{
      const v=sp.get(k),sel=document.getElementById(id);
      if(v&&[...sel.options].some(o=>o.value===v)){sel.value=v;lsState[k]=v;}});
    const qq=sp.get('q');if(qq){lsState.q=qq;document.getElementById('lsSearch').value=qq;document.getElementById('lsSearch2').value=qq;}
  }catch(e){}
  syncPresChips();syncRelChips();syncTierGroup();
  document.querySelectorAll('#lsFilters .chip[data-tier]').forEach(x=>x.setAttribute('aria-pressed',String(x.dataset.tier===lsState.tier)));
  LS_TABLE._setCols(lsPickCols());
  LS_TABLE._render();
}
...
/* ---------- share / text ---------- */
(function(){
  const sb=document.getElementById('shareBtn'),tb=document.getElementById('textBtn');
  const sbHTML=sb.innerHTML; let sbT=null;
  const copied=()=>{sb.innerHTML='✓ Link copied';clearTimeout(sbT);sbT=setTimeout(()=>{sb.innerHTML=sbHTML;},2000);};
  const taCopy=u=>{try{const ta=document.createElement('textarea');ta.value=u;ta.setAttribute('readonly','');ta.style.cssText='position:fixed;left:-999px;top:0';document.body.appendChild(ta);ta.select();const ok=document.execCommand('copy');ta.remove();return ok;}catch(e){return false;}};
  sb.addEventListener('click',()=>{
    const url=location.href;
    if(navigator.share){navigator.share({title:'AI Data Vendors · Competitive Landscape',text:'Competitive landscape — __ALL_TOTAL__ AI & data vendors',url}).catch(()=>{});return;}
    const manual=u=>{try{prompt('Copy this link:',u);}catch(e){}}; // last resort: clipboard denied + execCommand failed
  ...
  // Text via sms: — mobile only (desktop stays clean); iOS wants "sms:&body=", Android "sms:?body="
  ...
  tb.addEventListener('click',()=>{
    ...
    location.href='sms:'+sep+'body='+encodeURIComponent('Check out this vendor landscape: '+location.href);
  });
})();
